      ...
      if (this.current_map.item_positions.hasOwnProperty(item)) {
        ...
        for (let i = 0 ; i < positions.length; i++) {
          if (pos_equal(positions[i], hero_pos)) {
            current_status = item2description[item].text;
          ...
          for (let i = 0 ; i < positions.length; i++) {
            if (pos_equal(positions[i], hero_pos)) {
              if (item === '$') {
                ...
                current_status = description.text + pris[description.genre];
              } else if (current_map_name !== 'coop' || coins >= price) {
                inventory.push(item);

                if (current_map_name === 'coop') {
                  coins -= price;
                  current_status = description.text + achete[description.genre] + ' pour ' + price + '.-';
                } else {
                  current_status = description.text + pris[description.genre];
                }
    ...
    for (let y = 0; y < map_lines; y++) {
      for (let x = 0; x < char_per_line;) {
        ...
        if (this.current_map.tile2color !== undefined) {
          color = this.current_map.tile2color[val];
        }

        if (color === undefined) {
          color = tile2color[val];
        }
      ...
      if (this.inventory.length === 0) {
        ...
        for (const i in this.inventory) {
          if (this.inventory.hasOwnProperty(i)) {
            ...
            const coord = this.to_screen_coord(x, y);
            this.engine.text('[' + i + '] ' + item2description[item].text, coord, get_text_color());
            y++;
          }
        }
      }
